...
import (
	...
	amino_acid_codes "github.com/DaanV2/go-genetics/pkg/amino_acid_codes"
	"github.com/stretchr/testify/require"
)

func Test_Code_Data(t *testing.T) {
	for _, c := range amino_acid_codes.Codes() {
		t.Run(fmt.Sprintf("Code(%s)", c), func(t *testing.T) {
			d := c.Data()
			nc := amino_acid_codes.From(d)

			require.Equal(t, nc, c)

			str := c.String()
			nc, err := amino_acid_codes.Parse(str)
...
func Test_Code_All(t *testing.T) {
	...
	for b := range 256 {
		...
		code, err := amino_acid_codes.ParseByte(byte(b))
